...
export default function Index({ auth, flash }) {
    ...
    useEffect(() => {
        const getusers = async() => {
            const getres = await fetch("https://anjurypatawaran.online/api/ticket/searchBar")
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div className="flex justify-between items-center">
                        {
                            ...
                            :
                            <div>
                                <Filters allUsersData={ tickets.data } setFilteredData={ setFilteredData }/>
                            </div>
                        }
                    ...
                    </div>
                    {
                        ...
                        :
                         ...
                         <ul role="list" className="divide-y divide-gray-100">
                             {tickets.data.map(data => (
                                <li key={data.id} className="flex justify-between gap-x-6 py-5">
                                 <div className="flex min-w-0 gap-x-4">
                                     <div className="min-w-0 flex-auto">
                                         <p className="text-sm font-semibold leading-6 text-gray-900"> <Link key={ data.encrypted_id } href={route('ticket.show', data.encrypted_id )} > { data.title } </Link> </p>
                         ...
                         : null }
                            <Pagination getData={ tickets.links } getLength={ tickets }/>
                        </div>
                    }
                    </div>
                </div>
